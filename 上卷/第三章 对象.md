...
- `[[PUT]]`
该方法会在编辑对象属性的时候被触发, 该方法比想象中要复杂的多, 被触发后主要执行以下操作:
  - 属性是否是访问**描述符**? 如果是并存在 `setter`, 即调用 `setter`
...
- `[obj]`**hasOwnProperty** 只会检查属性是否在对象中, 不会检查 `[[Prototype]]` 链
...
和数组不同, 普通的对象没有内置**迭代器**, 传统对对象遍历的方法只能取出对象的 `key`(枚举), 要访问对象的值, 我们必须手动调用 `key` 来访问,
